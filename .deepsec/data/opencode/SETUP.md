# Agent setup for `opencode`

This is a deepsec scanning workspace. Project `opencode` was just registered
(target: `..`). Setup is incomplete — `data/opencode/INFO.md`
...
2. **Fill in `data/opencode/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id opencode
pnpm deepsec process --project-id opencode
```
