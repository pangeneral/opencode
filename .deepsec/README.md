...
Currently configured project: `opencode` (target: `..`).
...
3. Open the parent repo in your coding agent (Claude Code, Cursor, …)
   and have it follow `data/opencode/SETUP.md` to fill in
   `data/opencode/INFO.md`.
...
`--project-id opencode` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/opencode/`.
